...
// Subcomponente Input
const Input = ({ label, name, type = "text", required, step, value, onChange, placeholder, icon: Icon }: any) => {
  return (
...
export default function DashboardInstFuncionarios() {
  ...
  useEffect(() => {
    if (!cnpj) {
      ...
      // window.location.href = "/login-instituicao"; // Comentado para o mock
      return;
    }
  ...
  async function cadastrarFuncionario(e: React.FormEvent) {
    ...
    await supabase.from("funcionario").insert([
      { nome_fun: nome, cargo_fun: cargo, salario_fun: salario, cnpj: cnpj },
    ]).then(({ error }: any) => {
      if (error) {
        ...
        setCargo("");
        setSalario("");
        // Simulação de adição local para feedback imediato no mock
        setFuncionarios([...funcionarios, { cod_fun: Math.random(), nome_fun: nome, cargo_fun: cargo, salario_fun: salario }]);
      }
    });
  }
  ...
  async function excluirFuncionario(id: number) {
    ...
    await supabase
      ...
      .then(({ error }: any) => {
        ...
        else setFuncionarios(funcionarios.filter(f => f.cod_fun !== id));
      });
  }
    ...
    <div className="flex w-full h-screen bg-slate-50 font-sans text-slate-800 overflow-hidden">
      ...
      >
        ...
        <nav className="flex-1 px-3 py-4 space-y-2 overflow-y-auto custom-scrollbar">
          ...
          <SidebarItem to="/dashboard-instituicao/funcionarios" icon={Users} label="Funcionários" active isOpen={sidebarOpen} />
        ...
        <header className="bg-white h-20 shadow-sm flex items-center justify-between px-8 z-10 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-xs text-slate-500">Gerencie a equipe da sua instituição</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 w-full bg-slate-50">
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-96 bg-white rounded-2xl border border-dashed border-slate-300 text-slate-400">
              <Users size={48} strokeWidth={1} className="mb-4 text-slate-300" />
              ...
              <table className="w-full text-left border-collapse">
                <thead className="bg-slate-50 border-b border-slate-100">
                  <tr>
                    ...
                    <th className="py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider">Cargo</th>
                    <th className="py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider">Salário</th>
                    <th className="py-4 px-6 text-right text-xs font-bold text-slate-500 uppercase tracking-wider">Ações</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {funcionarios.map((f) => (
                    <tr key={f.cod_fun} className="hover:bg-slate-50 transition-colors group">
                      ...
                      <td className="py-4 px-6 text-slate-600">
                         <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-100 text-slate-600 text-xs font-medium">
                           <Briefcase size={12} /> {f.cargo_fun}
                         </span>
                      </td>
                      <td className="py-4 px-6 font-mono text-slate-600 text-sm">R$ {Number(f.salario_fun).toFixed(2)}</td>
                      <td className="py-4 px-6 text-right">
                        <button 
                          onClick={() => excluirFuncionario(f.cod_fun)}
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
           <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
              ...
              <form onSubmit={cadastrarFuncionario} className="p-6 space-y-5">
                 <Input label="Nome Completo" value={nome} onChange={(e: any) => setNome(e.target.value)} required placeholder="Ex: Maria Silva" icon={Users} />
                 <Input label="Cargo" value={cargo} onChange={(e: any) => setCargo(e.target.value)} required placeholder="Ex: Gerente" icon={Briefcase} />
                 <Input label="Salário (R$)" type="number" step="0.01" value={salario} onChange={(e: any) => setSalario(e.target.value)} required placeholder="0.00" icon={DollarIcon} />
